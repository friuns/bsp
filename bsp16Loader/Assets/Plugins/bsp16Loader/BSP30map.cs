...
namespace bsp
{
    public class BSP30Map
    {
        ...
        private BspInfo bspInfo; //info_player_start
        ...
        public BSP30Map(MemoryStream ms)
        {
            ...
            Debug.Log2("Entity char length " + entityLump.rawEntities.Length.ToString());
            Debug.Log2("number of Faces " + bspInfo.mapNum_faces.ToString());
            //   Debug.Log("number of edges " + bspInfo.
            Debug.Log2("lightmap length " + bspInfo.mapNum_lighting.ToString());
            Debug.Log2("number of verts " + bspInfo.mapNum_verts.ToString());
            Debug.Log2("number of Faces " + bspInfo.mapNum_faces.ToString());
            Debug.Log2("textures " + bspInfo.mapNum_textures.ToString());
            Debug.Log2("marksurf " + markSurfacesLump.markSurfaces.Length.ToString());
            Debug.Log2("VisData Length: " + bspInfo.mapNum_visability.ToString());
            Debug.Log2("leaf limp  Length: " + leafLump.numLeafs.ToString());
            Debug.Log2("plane Length: " + planeLump.planes.Length.ToString());
            Debug.Log2("node lump Length: " + nodeLump.nodes.Length.ToString());
            Debug.Log2("models " + modelLump.models.Length.ToString());

            Debug.Log2("data end ");
            bspInfo.mapNum_clipnodes = header.directory[9].length / 8;

            ReadPVS();
            var bytes = BSPfile.ReadBytes(3);

            var text = BSPfile.ReadNullTerminated();
            Debug.Log("vtf file len: " + text.Length);
            //Encoding.ASCII.GetString(BSPfile.ReadBytes((int)(BSPfile.BaseStream.Length - BSPfile.BaseStream.Position)));
        ...
        private void ReadTextures()
        {
            ...
            for (int indexOfTex = 0; indexOfTex < numberOfTextures; indexOfTex++)
            {
                ...
                Debug.Log2("starting to read in texture " + miptexLump[indexOfTex].name.ToString());
